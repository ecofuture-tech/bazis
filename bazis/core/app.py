"""
Main application module for the Bazis project, setting up the FastAPI application,
...
async def get_api_schemas(request: Request):
    ...
    for lang in expand_lang(to_locale(get_language())):
        ...
        if os.path.exists(lang_path):
            ...
            return RedirectResponse(f'{settings.STATIC_URL}schemas_{lang}.json?_h={lang_path_dt}')
...
def exc_encoder(errs: list[SchemaError], status: int, cookies: list[tuple[str, str, int]] = None):
    """
    ...
    return exc_encoder(
        [
            SchemaError(
                ...
                source=get_source_from_loc(err.loc, _id=get_item_id(err), _type=get_item_type(err)),
                meta=err.meta,
            )
        ...
        [
            SchemaError(
                ...
                source=get_source_from_loc(
                    err.get('loc'), _id=get_attr(err, 'ctx._id'), _type=get_attr(err, 'ctx._type')
                ),
            )
            for err in exc.errors()
        ],
...
async def json_api_http_exception_handler(request: Request, exc: HTTPException) -> JSONResponse:
    """
    ...
    return exc_encoder(
        [
            SchemaError(
                ...
                meta=meta,
                traceback=''.join(traceback.format_exception(exc)) if settings.DEBUG else None,
            )
        ],
        exc.status_code,
    )
...
async def json_api_http_exception_handler(request: Request, exc: Exception) -> JSONResponse:  # noqa: F811
    """
