...
class Division(DtMixin, UuidMixin, JsonApiMixin):
    ...
    def drivers_list1(self, dc: DependsCalc) -> list:
        return [
            ...
            for driver in dc.data.drivers1
        ]
...
class Vehicle(DtMixin, UuidMixin, JsonApiMixin):
    ...
    @calc_property([FieldDynamic('vehicle_model').add_nested([FieldDynamic('brand')])])
    ...
    def get_tasks_for_vehicle(self, vehicle_id):
        return CarrierTask.objects.filter(vehicle_id=vehicle_id, dt_finish=None).order_by(
            '-dt_start'
        )
